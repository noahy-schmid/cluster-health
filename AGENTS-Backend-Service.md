...
- `/src/application/errors.ts`: Shared base error types used across aggregates: `InternalError`, `NotFoundError`, `ConflictError`, `ValidationError`. Each aggregate re-exports only the errors it uses from its own `errors.ts` file.
